...
class OloraModel(LoraModel):
    ...
    def _create_new_module(self, lora_config, adapter_name, target, training):
        ...
        if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
            ...
            new_module = OloraLinear(adapter_name, in_features, out_features, 
...
class OloraLayer(LoraLayer):
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights):
        ...
        if r > 0:
            self.lora_A.update(nn.ModuleDict({adapter_name: OloraLinearA(self.in_features, r, self.expert_num, self.cur_task, self.training)}))
            self.lora_B.update(nn.ModuleDict({adapter_name: OloraLinearB(r, self.out_features, self.expert_num, self.cur_task, self.training)}))
...
class OloraLinear(nn.Linear, OloraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        OloraLayer.__init__(self, in_features=in_features, 
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        if self.disable_adapters:   # No adapter
            ...
            if self.training:
                ...
                lora_a_output = self.lora_A[self.active_adapter](self.lora_dropout[self.active_adapter](x), self.cur_task)
                lora_b_output = self.lora_B[self.active_adapter](lora_a_output, self.cur_task)
                result += lora_b_output * self.scaling[self.active_adapter]
        else:
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
...
class OloraLinearA(nn.Module):
    ...
    def forward(self, x, cur_task):
        ...
        if self.training:
            ...
            total_out_features = (cur_task + 1) * self.r
...
class OloraLinearB(nn.Module):
    ...
    def forward(self, x, cur_task):
        ...
        if self.training:
            ...
            total_in_features = (cur_task + 1) * self.r
