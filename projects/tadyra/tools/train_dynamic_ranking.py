...
def process_single_layer_svd(args):
    name, grad_data, lora_r, lora_scale = args
    grad = torch.from_numpy(grad_data) if not isinstance(grad_data, torch.Tensor) else grad_data
    ...
    try:
        # SVD分解
        q = min(10 * lora_r, min(grad.shape))
        U, S, V = torch.svd_lowrank(grad.float(), q=q, niter=2)
        ...
        # 因为lora被加载后需要*scale
        result.update({
            'lora_A': (lora_A / scale_div).cpu().numpy(),  # 转numpy便于序列化
            'lora_B': (lora_B / scale_div).cpu().numpy(),
            'success': True
        })
        error = (lora_B @ lora_A.T - grad.float()).norm(p=2)
        print(f"layer {name}, error: {error}")
...
def parallel_lora_init(updates, ranks_map, lora_scales, num_workers=4):
    ...
    for name, lora_r in ranks_map.items():
        if lora_r > 0 and name in updates:
            ...
            grad_np = grad.cpu().numpy() if isinstance(grad, torch.Tensor) else grad
            scale = float(lora_scales.get(name, 1.0))
            tasks.append((name, grad_np, lora_r, scale))
...
def main():
    ...
    visualizer = dict(
        ...
        vis_backends=[dict(
                ...
                init_kwargs=dict(project='cvit', experiment_name=f'{osp.basename(args.config)}/task{args.cur_task}_before_train')
            )]
            )
    ...
    if cfg.get("framework", "mmengine").lower() == "huggingface":
        ...
        ###############################################################
        #          train both projector and naive lora                #
        ###############################################################

        cfg.model.llm_lora = cfg.llm_lora_base
        cfg.train_dataloader.dataset.sample_ratio = 0.10
        ...
        cfg.work_dir_parent = cfg.work_dir
        cfg.work_dir = osp.join(cfg.work_dir_parent, f"task{args.cur_task}")
        ...
        from mmengine._strategy.deepspeed import MMDeepSpeedEngineWrapper
        from mmengine.dist import is_main_process
        if isinstance(runner.model, MMDeepSpeedEngineWrapper):
            model = runner.model.model.module
        else:
            model = runner.model

        ###############################################################
        #                    STEP2: dynamic ranking                   #
        ###############################################################
        from peft.tuners.dymorelora import DyMoeLoraLayer
        from peft.tuners.lora import LoraLayer

        if is_main_process():
            llm_with_lora = model.llm
            
            # 收集updates和scales
            updates = {}
            lora_scales = {}
            for key, module in llm_with_lora.named_modules():
                if isinstance(module, LoraLayer):
                    delta_weight = module.get_delta_weight(module.active_adapter)
                    updates[key] = delta_weight.detach().cpu()
                    lora_scales[key] = module.scaling[module.active_adapter]
            
            # 计算scores
            scores = {key: grad.norm(p=2) for key, grad in updates.items()}
            scores_sum = sum(scores.values())
            
            # 参数
            r_base = cfg.llm_lora.r
            ...
            for name, score in tqdm(scores.items(), desc="Computing ranks"):
                lora_r = math.ceil(score / scores_sum * r_base * L * scale)
            ...
            for key, grad in updates.items():
                ...
                total_params_base += (d_in+d_out) * r_base
            ...
            # combine with previous lora  init
            pre_path = os.path.join(cfg.work_dir_parent, f'task{args.cur_task-1}/dyra_init.pt')
            ...
            save_path = os.path.join(cfg.work_dir_parent, f'task{args.cur_task}/dyra_init.pt')
