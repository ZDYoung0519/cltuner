...
class ToyModel(BaseModel):
    def __init__(self, visual_encoder, text_encoder, text_tokenizer, router_args={}, cur_task=0):
        super().__init__()
...
def main():
    ...
    # )
    cfg.model = dict(
        type="projects.dyra.tools.train_dynamic_routing.ToyModel",
        visual_encoder=cfg.model['visual_encoder'],
        text_encoder=cfg.model['text_encoder'],
        text_tokenizer=cfg.model['text_tokenizer'],
        router_args=cfg.model['router_args'],
    ...
    if args.cur_task > 0:
        # load previous ckpt
        ckpt = osp.join(work_dir_parent, f'task{args.cur_task-1}/router_best.pth')
        if osp.exists(ckpt):
        ...
        for epoch in range(cfg.max_epochs):
            ...
            if is_main_process():
                ...
                if val_acc1 > best_acc:
                    ...
                    torch.save(state_dict, osp.join(work_dir_parent, f'task{args.cur_task}/router_best.pth'))


        if is_main_process():
            state_dict = model.state_dict()

            torch.save(state_dict, osp.join(work_dir_parent, f'task{args.cur_task}/dynamic_routing_last.pth'))
